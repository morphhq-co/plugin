...
1. **Start or resume:** before substantive Brain discovery or writes, call `list_skills`, then call `load_skill` for the current organization-authored `brain-read-write` skill when available; the live Morph package is the source of truth. Load any other organization skill relevant to the task. Then identify the user and organization, read `brain/hooks/session-start.md` when it exists, and search Morph for existing task and repository context.
